...
## 11. Token source, fixtures, service contracts and the component gallery (implemented)

This section records what the code now is, so the next step can build on it instead of
re-deriving it.

### 11.1 The token source

`design/tokens.json` is the single token source. It is generated by
`.mobile-work/generate-tokens.py`, which reads `shared-spec/design-tokens.json`,
`shared-spec/motion-language.md`, this document and the Compose token files, then:

- resolves every upstream alias for both themes (`neutral.50` is bone on Paper and
  carbon on Carbon);
- turns each alpha modifier into an explicit alpha and an 8-digit ARGB literal;
- names the functional roles the components need, because upstream defines only
  background, text and separator;
- measures WCAG contrast for the 27 pairs the UI actually uses, and records the
  result in `verify.contrast`. Six upstream alias choices could not reach their
  target; the role moved to the nearest upstream step that passes and carries a
  `note` explaining the move. No pair is left failing.

Platform mapping, one row per token group:

| Token group (`design/tokens.json`) | Compose file | Rule |
| --- | --- | --- |
| `color.semantic.*` | `core/designsystem/color/HeliosSemanticColor.kt` | `HeliosSemanticColors` with `PaperColors` and `CarbonColors`. Provided through `LocalHeliosSemanticColors` by `HeliosTheme`. Screens read `HeliosThemeTokens.colors`. |
| `color.hues.*` | `core/designsystem/color/HeliosColor.kt` | Unchanged ramps; the only source of hue values. |
| `accent.*` | `HeliosSemanticColors.withBrandAccent` | A white-label brand replaces the accent roles only. Status, insight, solar, flow, battery, grid and alert roles are never touched. |
| `color.semantic.*` to Material | `core/ui/theme/HeliosTheme.kt` | `heliosLightScheme` / `heliosDarkScheme` map semantic roles to `ColorScheme` for Material components only. |
| `typography.*` | `core/designsystem/type/HeliosTypography.kt` | Ten scale tokens plus the chart and ticker numeric variants. |
| `spacing.*`, `layout.*` | `core/designsystem/layout/HeliosSpacing.kt` | 4 dp grid, gutter 20, section rhythm 20, card padding 16, 48 dp touch target, and `LayoutMetrics` for the hero block, charts and nav. |
| `radius` | `core/designsystem/shape/HeliosShape.kt` | `full` is a pill (percent 50), not 50 dp. |
| `elevation.*` | `core/designsystem/shape/HeliosElevation.kt` | Levels 0-5 plus the material tonal elevations and the 0.5 dp hairline inner stroke. |
| `motion.*` | `core/designsystem/motion/HeliosMotion.kt` | Four spring presets, durations in both milliseconds and seconds, the four named curves (linear is for loaders and the shimmer only), entrance delays and the Reduce Motion contract. |
| `haptics.*` | `core/designsystem/haptics/HeliosHaptics.kt` | Unchanged; mappings already match the token file. |
| `verify.contrast` | `design/tokens.json` | The measurement, not a claim: 27 pairs, 0 failures. |

### 11.2 Fixtures and the service contracts

`core/data/service/**` holds the interfaces; `core/data/fixture/**` holds the offline
adapters; `core/data/service/DeviceServices.kt` holds the adapters that wrap the
existing repositories. `ServiceGraph.current` returns the fixture family until the app
installs the device family, which is the swap point a later step moves to Hilt.

| Contract | Interface | Fixture adapter | Device adapter |
| --- | --- | --- | --- |
| telemetry (SVC-01) | `TelemetryService` | `FixtureTelemetryService` | `RepositoryTelemetryService` |
| today series (SVC-02) | `SeriesService.todaySeries` | `FixtureSeriesService` | `RepositorySeriesService` |
| live series | `SeriesService.liveSeries` | `FixtureSeriesService` | `RepositorySeriesService` |
| week series (SVC-03) | `SeriesService.weekSeries` | `FixtureSeriesService` | `RepositorySeriesService` |
| forecast (SVC-04) | `ForecastService` | `FixtureForecastService` | `RepositoryForecastService` |
| insights (SVC-07, SVC-09) | `InsightsService` | `FixtureInsightsService` | `RepositoryInsightsService` |
| connection (SVC-19) | `ConnectionService` | `FixtureConnectionService` | `InMemoryConnectionService`, labelled interim: it does not persist until the Room repository exists |
| location (SVC-05) | `LocationService` | `FixtureLocationService` | `FusedLocationService` |
| theme (SVC-18) | `ThemeService` | `FixtureThemeService` | `DataStoreThemeService` |
| brand (SVC-10, SVC-11) | `BrandService` | `FixtureBrandService` | `RegistryBrandService` |
| share (SVC-12 to SVC-15) | `ShareService` | `FixtureShareService` | `RepositoryShareService` |

Every call returns a `Loadable`: `Loading`, `Ready(value, SourceMeta)`, `Empty(meta,
message, action)`, or `Failed(ServiceFailure)`. `Loadable.uiState()` maps that to the
eight view states, and `ServiceFailure.kind` is the FLW-03 taxonomy F1-F9, so the words
and the offered action come from the failure class rather than from a screen.

The nine demo scenarios live in `FixtureScenario`: Loading, Live, Empty, Partial, Stale,
Offline, Error, Denied permission and Long content. Switching one flag moves every
adapter at once, which is what makes a state review possible without hardware.

`core/format/HeliosFormat.kt` is the one formatting source (SVC-17). A register read
that failed is `Double.NaN` and renders as "No data"; a real 0 renders as 0.

### 11.3 The component gallery

`android/app/src/debug/**` holds the gallery, so no release build can contain it:

- `ComponentGalleryScreen` is the composable entry point. It takes a page, a scenario,
  a theme and a demo flag, and it composes no navigation.
- `GalleryActivity` is a debug-only host declared in `src/debug/AndroidManifest.xml`,
  reachable with `am start -n com.helios.app/com.helios.debug.gallery.GalleryActivity
  --ei page 3 --ez dark true --es scenario OFFLINE`.
- `GallerySections.kt` renders every reused component in every variant and state:
  energy flow (live, night, stale, offline, reduced motion), battery ring (charging,
  discharging, idle, below 20 percent, no data), production chart (ready with scrub,
  empty before sunrise, loading, stale), week chart (ready, selected, loading, empty,
  error), metric tile (default, sparkline, both delta signs, subtle, disabled, no
  data, long label), live number (updating, static, stale, demo), status pill (all
  seven), freshness stamp, insight highlight and card (four severities, with and
  without action, demo qualifier, long body), forecast strip and card (ready, loading,
  empty, stale, error), section header, weather icon (ten conditions), brand marks,
  bottom nav (five destinations, every selection, badge), top bar (connected, demo,
  stale, offline, white label), buttons and chips (default, loading, disabled with its
  precondition), share sheet (idle, copied, failed), empty, error, denied-permission
  and skeleton states, connection banner (offline, reconnecting, recovered), live trail
  (running, stale, empty), and a contract table for all nine services.
- `GalleryPaging.kt` packs the sections into pages that fit one screen: 47 sections,
  47 pages at the 380 dp content budget. Every page ends with a magenta stripe; a capture
  without that stripe means the page overflowed, and the fix is a smaller page, not a
  re-capture. Five pages overflowed at the first budget and were split rather than
  re-captured.

Capture and verification commands:

```sh
bash .mobile-work/boot-emulator.sh          # holds /tmp/helios-emulator.lock
cd android && ./gradlew :app:assembleDebug
bash .mobile-work/capture-gallery.sh        # one PNG per page
python3 .mobile-work/build-gallery-sheet.py # marker check + contact sheet
```

Evidence lives in `design/captures/`: one `gallery-pageNN.png` per page, the capture log
(`gallery-capture.txt`), the marker check and colour counts (`gallery-check.txt`), two
extra single-screen captures (the Paper theme and the offline scenario), and
`gallery-components.png`, the two-column contact sheet of every page at 40 percent scale.

What the captures prove: every page rendered, every page fitted one 1080x2400 screen
(the marker is present in all 47), and no frame is blank. What they do not prove: that the
layout is good, or that any of it holds at another text scale. That review is the next
step's, and it needs eyes, not a script.
