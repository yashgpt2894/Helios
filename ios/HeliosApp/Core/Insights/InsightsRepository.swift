...
enum InsightsRepository {
    ...
    static func generateForecastInsights(forecast: ProductionForecast?, t: SolarTelemetry) -> [Insight] {
        ...
        if projectedSavings > 10 {
            insights.append(Insight(
                ...
                body: "If forecast holds and self-consumption stays near 70%, you'll avoid roughly $\(String(format: "%.0f", projectedSavings)) in grid imports over the next 7 days.",
                metric: "$\(String(format: "%.0f", projectedSavings))",
                delta: "next 7 days",
                actionLabel: nil
            ))
        }
    ...
    static func computeSavings(t: SolarTelemetry) -> (today: Double, month: Double, lifetime: Double) {
        return (
            t.energyTodayKwh * kwhRateGrid,
            t.energyMonthKwh * kwhRateGrid * 0.85,
            t.energyLifetimeKwh * kwhRateGrid * 0.8
        )
    }

    // MARK: - Private helpers

    private static func computeStringSpread(_ t: SolarTelemetry) -> Double {
        guard !t.panels.isEmpty else { return 0 }
        let expected = t.panels.map { $0.powerW / $0.ratedW }
        guard let max = expected.max(), let min = expected.min(), max > 0 else { return 0 }
        return (max - min) / max
    }
    ...
    private static func friendlyDay(_ dateStr: String) -> String {
    ...
    private static func bestDay(_ days: [ForecastDay]) -> String {
        guard let best = days.max(by: { $0.expectedKwh < $1.expectedKwh }) else { return "N/A" }
        return friendlyDay(best.date)
    }

    private static func worstDay(_ days: [ForecastDay]) -> String {
        guard let worst = days.min(by: { $0.expectedKwh < $1.expectedKwh }) else { return "N/A" }
